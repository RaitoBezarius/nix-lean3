...
def find_all_oleans(p: Path) -> Generator[Path, None, None]:
    for root, _, files in os.walk(p.absolute()):
        for file in files:
...
lean_version_raw = subprocess.run(
    ["lean", "-v"], capture_output=True, encoding="utf-8"
).stdout.strip()
# do not use githash, it is unnecessary.
lean_version_match = re.search("version ([0-9]+.[0-9]+.[0-9]+)", lean_version_raw)
if not lean_version_match:
    raise RuntimeError('Lean is not versioned properly, cannot proceed.')
lean_version = f"v{lean_version_match.group(1)}"
lean_githash_match = re.search("commit ([a-z0-9]{12}),", lean_version_raw)
lean_githash = None
if lean_githash_match:
    lean_githash = lean_githash_match.group(1)
print(f"Raw version string: {lean_version_raw}, detected version: {lean_version}, detected git hash: {lean_githash}")
# assume leanprover-community repo
core_url = (
    "https://raw.githubusercontent.com/leanprover-community/lean/{0}/library/"
    .format(lean_githash or lean_version)
)
...
with zipfile.ZipFile(
    ...
    for p in lean_path:
        ...
        if str(p.resolve()) == combined_lib_path:  # if using combined_lib/src
            ...
            print(f'[+] {lib_name} → {lib_info[lib_name]} (combined library)')
            ...
            print(f'[+] {lib_name} → {lib_info[lib_name]} (library)')
        else:
            lib_name = core_name
            lib_info[lib_name] = core_url
            print(f'[+] {lib_name} → {lib_info[lib_name]} (Lean core library)')
        if lib_name not in num_olean.keys():
            num_olean[lib_name] = 0
        for fn in find_all_oleans(p):
