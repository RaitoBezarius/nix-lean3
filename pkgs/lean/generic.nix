...
let
  drv = { checkOleanVersion ? true, enableAdvancedLogging ? false }: stdenv.mkDerivation (finalAttrs: {
    ...
    patches = [
      ...
      (if lib.versionAtLeast version "v3.42" then ./patches/0003-emscripten-logging-for-oleans-for-3_42_and_later.patch else ./patches/0003-emscripten-add-some-logging.patch)
