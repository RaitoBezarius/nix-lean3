...
let
  ...
  drv = { useOldOleans ? false }: buildEmscriptenPackage rec {
    ...
    ++ lib.optional enableAdvancedLogging (if lib.versionAtLeast version "v3.42" then ./patches/0003-emscripten-logging-for-oleans-for-3_42_and_later.patch else ./patches/0003-emscripten-add-some-logging.patch);
    ...
    configurePhase = ''
      ...
      emcmake cmake -S src -B $TMPDIR/build ${lib.optionalString (!checkOleanVersion) "-DCHECK_OLEAN_VERSION=OFF"} -DCMAKE_BUILD_TYPE=Debug
    '';
